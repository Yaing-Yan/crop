...
## ✅ 优化 1 已实现：字符串骑链（114 → **66 字节**，真机验证通过）

**做法**（与下面的原始设计略有不同，更稳）：

* 载体 gadget **扫描得到**：`POP QR8 ; POP QR0 ; POP PC`（VerF `0x10F14`，一次吃 **16 字节**、
  不写内存、无副作用）；
* 字符串**统一放在程序末尾**（链尾）而不是调用点：ROM 例程的 `PUSH`/栈帧只占 **SP 以下**
  的"已走过"链段，链尾在 SP 之上 ⇒ **不会被踩**（最初放在调用点时确实被踩，VM 里画不出字）；
* 实参处发**哨兵** `0x8000 | idx`（idx = 该字符串在"骑链表"里的序号），解释器遇到哨兵就
  换成 **该载体数据在链上的真实地址**（用 `ChainBuilder.anchor()` + `$Pxxxx` 前向引用，
  机制现成）；不是哨兵的地址照旧按字面地址用 ⇒ 没有歧义；
* 长字符串（> 载体容量）自动回退到原来的"数据区 + 块写"路径，不回归。

**实测**：`examples/hello_world.c` 链长 **114 → 66 字节**（VerF/VerC 都是），
0 条不支持；DSL 与直接构造字节一致（无告警）。
...
链 66 字节 @0xD400 回读一致 ✓
D137 = 0x0E
0xDDD4 屏幕缓冲区非零 119 字节；点阵解码就是 HELLO WORLD!
F800[32:56] == DDD4[24:48] ✓   F800[64:88] == DDD4[48:72] ✓（rrefresh 的搬运正确）
```

> 注意：**离线 lifted 模拟器里这条链没画出字**（`D137=0E` 但缓冲区空）——
> 那是模拟器对渲染器深处代码的提升精度问题（历史上多次出现）；真机内存证据为准。
> 另外 `print-0x1y` 的 `x` 参数看起来不像"像素偏移"（我传 30，点阵却从 x≈0 起），
> 待用一张专门的测试图确认它是"列/半角单位"还是别的编码。

## 优化 1（原始设计，保留备查）：字符串骑链（省掉整块初值写入）
