...
| **VerC (second ROM version)** | same `examples/hello.c`, only `--rom-dir` changed | ✅ compiles to a 128-byte chain with `unsupported == 0`; labels resolve to VerC's `0x221B2`/`0x08706`/`0x07F00`, rt-fix to `0x2B948`; on the emulator the read-back shows a rendered text line (8.1 % non-zero) and `F800[32:56] == DDD4[24:48]` |
